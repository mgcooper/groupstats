function tbl = settablevarnames(tbl,varnames,varargin)
   ...
   %  value in VARNAMES for each column of tbl.
   %
   %  tbl = settablevarnames(tbl, VARNAME, 'consecutive') sets all variable names in
   %  table tbl to VARNAME with the column number appended. In this mode, VARNAME can
   %  be a scalar string, cell array, or a char array, and the column number is
   %  appended such that the variable names of tbl are VARNAME_1, VARNAME_2, ...,
   %  VARNAME_N.
   %
   % See also

   % parse inputs
   validateattributes(tbl, {'table', 'timetable'}, {'nonempty'}, mfilename, 'tbl', 1)
