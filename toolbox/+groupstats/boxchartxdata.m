function [xlocs,xleft,xright] = boxchartxdata(H)
   ...
   %
   % More detail: size(xlocs) = numel(cgroupvars) x numel(xgroupvars) = M x N
   % size(H) = numel(cgroupvars) x 1 = M x 1
   %
   % i.e., for a boxchart figure with: N = numel(xgroupvars) (number of xticks),
   % and M = numel(cgroupvars) (number of boxcharts per xtick),
   %
   % size(H) = M x 1 (number of boxcharts per xtick BY 1) size(xlocs) = M x N
   % (number of boxcharts per xtick BY number of xticks)
   %
   %
   ...
   % This might be helpful, but it's notch on that matters any(arrayfun(@(n)
   % iscategorical(H(n).XData),1:numel(H)))
   ...
   % If every xtick had the same number of boxcharts: N =
   % numel(H(1).NodeChildren(5).VertexData(1,:))/numVertsPerBox;
   ...
   for m = 1:M
      ...
      % above works, but instead of 1:N(m), get the actual indices I think this
      % only works b/c the x-axis data is already numeric in this
      ...
      % think I may have a 'hold on' somewhere that i need to turn off so
      % repeeated calls overwrite it, ratehr than deal with it here. This might
      % be needed, where xmean=round(mean(xverts(1:2,:))) Find which
      % x-coordinates are close to the rounded mean. I think this would deal
      % with the case where the boxcharts extend over the halfway mark b/w
      % xticks, but I don't think that's possible xmean =
      % round(mean(xverts(1:2,:))); xidx = abs(xverts(1,:) - xmean) < 0.5;
      % xlocs(m,xidx) = mean(xverts(1:2,:)); Then xidx would be used like:
...
% xverts1 = double(reshape(H(1).NodeChildren(5).VertexData(1,:),8,[])); xvertsM
% = double(reshape(H(M).NodeChildren(5).VertexData(1,:),8,[])); xleft =
% xverts1(1,:); xright = xvertsM(2,:);


% Part 1 without initialization (work backwards): for n = numel(H):-1:1
%    xVertexData = double(reshape(H(n).NodeChildren(5).VertexData(1,:),8,[]));
%    xlocs(n,:) = mean(xVertexData(1:2,:));
% end

% Better to leave in matrix form for plotting against grouped data xlocs =
% xlocs(:);


%% For reference, using fun

% % This gets the xverts xverts = arrayfun( @(m) ...
%    double(reshape(H(m).NodeChildren(5).VertexData(1,:),8,[])), ...
%    1:M,'uni',0);
%
% % This gets the xlocs xlocs = cellfun(@(x) mean(x(1:2,:)), xverts ,'uni',0);
%
% % This gets the indices xnodes = cellfun(@(x) unique(round(x)), xlocs, 'uni',
% 0);
%
% % This is a blank 1:9 vector that could be used to fill in V = (1:max(N))';
%
% % This gets the indices without computing 'xnodes' above I = cellfun(@(x)
% ismember((1:max(N))',x'), cellfun(@(x) ...
%    unique(round(x)), xlocs,'uni',0),'uni',0);
% I = horzcat(I{:});
%
% % This gets the indices in one 'line' I = cellfun(@(x)
% ismember((1:max(N))',x'), ...
