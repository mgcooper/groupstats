function varargout = backupfile(filename, makecopy, makezip)
   ...
   % Octave has no isfolder, isfile, or convertStringsToChars, and neither
   % does MATLAB before R2017b. Short circuit, so Octave never reaches
   % verLessThan.
   ...
   % Assign the outputs only when the caller asks for them. Fixed outputs
   % would print ans on a call that requests none.
   switch nargout
      case 1
...
function filedate = mkfiledate(dateformat)
   % Defined here, so this file calls nothing outside its own folder.
   if nargin < 1
