function F = privatefunction(funcName)
   ...
   % Match a folder named private, not any path holding that word. On macOS
   % the real paths /private/tmp and /private/var exist, and tempdir sits
   % under one, so a substring match takes every folder in those trees and
   % cell2struct then raises DuplicateFieldName on the parent names.
   [~, foldernames] = cellfun(@fileparts, allfolderslist, ...
      'UniformOutput', false);
   allprivatefolders = allfolderslist(strcmp(foldernames, 'private'));
   ...
   if isempty(F)
      if nargin == 1
         error('groupstats:privatefunction:functionNotFound', ...
            'Function %s not found in any private directories.', funcName);
      elseif nargin == 0
         error('groupstats:privatefunction:noFunctionsFound', ...
